...
export type CapsuleListQueryVariables = Exact<{ [key: string]: never; }>;


export type CapsuleListQuery = (
  { __typename?: 'Query' }
  & { capsules?: Maybe<Array<Maybe<(
    { __typename?: 'Capsule' }
    & { dragon?: Maybe<(
      { __typename?: 'Dragon' }
      & Pick<Dragon, 'active' | 'crew_capacity' | 'description' | 'dry_mass_kg' | 'dry_mass_lb'>
    )> }
  )>>> }
);
...
export const CapsuleListDocument = gql`
    query capsuleList {
  capsules {
    dragon {
      active
  ...
  export class CapsuleListGQL extends Apollo.Query<CapsuleListQuery, CapsuleListQueryVariables> {
    document = CapsuleListDocument;
